...
struct Slider { std::string name; float* val = nullptr; float lo = 0, hi = 1; int side = 0; bool intStep = false; int prec = 2; bool reactive = false; int show = 0; int tab = -1; ofRectangle track; std::vector<std::string> opts; std::vector<ofRectangle> boxes; bool icons = false; };  // show: 0 always·1 radial·2 grid · tab: -1 left/always · 0 LAYOUT · 1 AUDIO right-tab · opts=>radio · icons=>shapes
struct Field  { std::string label; std::string* sp = nullptr; int* ip = nullptr; float* fp = nullptr; std::string buf; ofRectangle box; std::vector<std::string> choices; std::string* op = nullptr; };  // editable session field (choices != empty => selector; op set => last choice "OTHER" is a free-text field)
...
public:
    ...
    float cfgFill    = 0;       // 0 = outline, 1 = filled (Fill)
