...
// ===========================================================================
#ifdef __APPLE__
  #define FFMPEG    "/opt/homebrew/bin/ffmpeg"                                   // Homebrew (GUI apps don't inherit PATH)
  #define GS_VCODEC "-c:v h264_videotoolbox -b:v 100M -pix_fmt yuv420p"          // GPU encode (VideoToolbox)
#else
  #define FFMPEG    "ffmpeg"                                                      // from PATH (Linux/Windows)
...
// session.json / recordings / scratch files: macOS uses the original project layout;
// other platforms use the portable app data folder (bin/data), created on demand.
static std::string gsSessionPath() {
#ifdef __APPLE__
    return "/Users/ishi/Music/heliograph/session.json";   // project root (no sessions/ folder)
#else
    return ofToDataPath("session.json", true);
#endif
}
static std::string gsPresetsDir() {
#ifdef __APPLE__
    std::string d = "/Users/ishi/Music/heliograph/presets/";
#else
    std::string d = ofToDataPath("presets/", true);
#endif
    ofDirectory::createDirectory(d, false, true);   // created on demand
    return d;
}
static std::string gsRecDir() {
#ifdef __APPLE__
    std::string d = "/Users/ishi/Music/heliograph/recordings/";
#else
    std::string d = ofToDataPath("recordings/", true);
#endif
    ofDirectory::createDirectory(d, false, true);   // recreate on demand (folder is .gitignored)
    return d;
}
...
void ofApp::buildSliders() {
    ...
    auto add = [&](std::string n, float* v, float lo, float hi, bool isInt, int prec, int tab, int show) {
        Slider s; s.name = n; s.val = v; s.lo = lo; s.hi = hi; s.intStep = isInt; s.prec = prec; s.tab = tab; s.show = show; sliders.push_back(s);
    };
    auto addChoice = [&](std::string n, float* v, std::vector<std::string> opts, int show) {
        Slider s; s.name = n; s.val = v; s.lo = 0; s.hi = (float)opts.size() - 1; s.intStep = true; s.prec = 0; s.show = show; s.tab = 0; s.opts = opts; sliders.push_back(s);
    };
    ...
    addChoice("Fill", &cfgFill, {"OUTLINE", "FILL"}, 0);
...
void ofApp::drawGrid() {
    ...
    if (cfgFill > 0.5f) ofFill(); else ofNoFill();
    ...
    for (int j = -cy; j <= cy; j++) {
        ...
        for (int i = -cx; i <= cx; i++) {
            ...
            if (sides == 0) ofDrawCircle(0, 0, base);
            else { ofBeginShape(); for (int k = 0; k < sides; k++) { float a = TWO_PI * k / sides - HALF_PI; ofVertex(cosf(a) * base, sinf(a) * base); } ofEndShape(true); }
            ofPopMatrix();
        }
        ofPopMatrix();
    }
...
void ofApp::drawPortal() {
    ...
    for (int i = 0; i < nb; i++) {
        ...
        if (cfgGlow > 0.01f && !filled) {                        // BLOOM: additive halo — a few progressively wider, dimmer strokes under the crisp line
            ofEnableBlendMode(OF_BLENDMODE_ADD);
            for (int p = 3; p >= 1; p--) {
...
void ofApp::drawPanels() {
    ...
    if (modPickKind >= 0) {                                  // pick-mode banner
        ofSetColor(232, 238, 234);
        std::string s1 = std::string(modPickKind == 0 ? "AUDIO MOD" : "LFO MOD") + " · click a highlighted control on the left to bind   ·   ESC to cancel";
        fUI.drawString(s1, RW * 0.5f - fUI.stringWidth(s1) * 0.5f, fm + 96 * S);
    }
...
void ofApp::drawSettings() {
    ...
    ofSetColor(150, 156, 154); fUI.drawString("RECORDING:   " + recBaseName() + ".mp4  +  .wav  (lossless audio)", px + 40 * S, py + ph - 104 * S);
...
void ofApp::stopRecording() {
    ...
    std::string vidTmp = b + ".video.mp4", wavPath = b + ".wav", log = gsScratch("gs_ffmpeg.log");
    { std::lock_guard<std::mutex> lock(mtx); writeWav(wavPath, recAudio, recChannels, sampleRate); }   // lossless WAV — kept
    // mux the upload MP4 (AAC for portability); the lossless .wav stays alongside it
    std::string mux = std::string(FFMPEG) + " -y -i \"" + vidTmp + "\" -i \"" + wavPath +
        "\" -c:v copy -c:a aac -b:a 320k -shortest \"" + recFinalPath + "\" 2>>\"" + log + "\"";
    system(mux.c_str());
    ofFile::removeFile(vidTmp);   // portable cleanup (no shell 'rm')
    ofLogNotice() << "REC saved -> " << recFinalPath << "   (+ lossless WAV: " << wavPath << ")";
}
...
void ofApp::keyPressed(int key) {
    ...
    if (key == OF_KEY_ESC) { if (recWarn) recWarn = false; else if (modPickKind >= 0) modPickKind = -1; else showHelp = false; return; }
...
void ofApp::mousePressed(int x, int y, int button) {
    ...
    if (settingsOpen) {
        for (size_t i = 0; i < fields.size(); i++)
            if (fields[i].box.inside(fx, fy)) {
                if (!fields[i].choices.empty() && fields[i].ip) {        // selector: click cycles
    ...
    if (rightTab == 2) {
        ...
        for (int gi = 0; gi < 6; gi++) {
            ...
            if (m.dest >= 0 && modClearBox[gi].inside(fx, fy)) {                  // clear a slot (restore its dest's value)
                if (m.dest < (int)sliders.size() && sliders[m.dest].val) *sliders[m.dest].val = m.base;
                m.dest = -1; return;
            }
    ...
    if (modPickKind >= 0) {
        for (size_t i = 0; i < sliders.size(); i++) {
            ...
            if (hit.inside(fx, fy)) {
                ...
                m.dest = (int)i; m.base = *s.val; m.amt = s.hi;                  // default target = the param's max
    ...
    for (size_t i = 0; i < sliders.size(); i++) {
        ...
        if (!s.opts.empty()) {                                       // radio buttons: click an option
            for (size_t b = 0; b < s.boxes.size(); b++)
                if (s.boxes[b].inside(fx, fy)) { *s.val = (float)b; return; }
            continue;
        }
