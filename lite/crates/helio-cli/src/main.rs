...
mod ui {
    ...
    pub fn amber(s: &str) -> String { paint(AMBER, s) }
    pub fn red(s: &str) -> String { paint(RED, s) }
    pub fn white(s: &str) -> String { paint(WHITE, s) }
    pub fn grey(s: &str) -> String { paint(GREY, s) }
    pub fn dim(s: &str) -> String { paint(DIMC, s) }
    pub fn bold(s: &str) -> String { paint(BOLD, s) }
}

#[derive(Parser)]
#[command(name = "helio", version, about = "Control the heliod broadcast appliance")]
struct Cli {
...
enum CollectionCmd {
    ...
    /// Rename a collection (--name), set its --description, or retitle a track (--track --title).
    Edit {
        id: String,
        #[arg(long)]
        name: Option<String>,
...
struct MetaArgs {
    ...
    #[arg(long)] episode: Option<u32>,
    #[arg(long)] font: Option<String>,
    #[arg(long)] accent: Option<String>,
    #[arg(long)] btc: Option<String>,
    #[arg(long)] ln: Option<String>,
    #[arg(long)] lq: Option<String>,
}
...
struct ImageArgs {
    ...
    #[arg(long)] clear: bool,
    #[arg(long, allow_hyphen_values = true)] pan_x: Option<f32>,
    #[arg(long, allow_hyphen_values = true)] pan_y: Option<f32>,
    #[arg(long)] opacity: Option<f32>,
    #[arg(long)] blur: Option<f32>,
    #[arg(long)] scale: Option<f32>,
}
...
fn main() -> Result<()> {
    ...
    match cli.cmd {
        Cmd::Status { watch } => {
            if watch {
                loop {
                    ...
                    match http.get(url("/status")).send().and_then(|r| r.json::<Status>()) {
                        Ok(s) => print!("{}", render_status(&s)),
                ...
                let s: Status = http.get(url("/status")).send()?.json().context("daemon unreachable")?;
        ...
        Cmd::Device { name } => ack("input", http.post(url("/device")).json(&serde_json::json!({ "device": name })).send()?)?,
        Cmd::Register { invite, artist, server } => {
            let body = RegisterRequest { invite_code: invite, artist, server };
        ...
        Cmd::Presence { state } => {
            ...
            if a.ok {
                let label = if away { ui::amber("AWAY") } else { ui::green("ON DECK") };
        ...
        Cmd::Meta(m) => {
            let body = MetaUpdate {
                channel: m.channel, title: m.title, note: m.note, date: m.date, episode: m.episode,
                font: m.font, accent: m.accent, btc: m.btc, ln: m.ln, lq: m.lq,
            };
            ack("metadata", http.post(url("/meta")).json(&body).send()?)?;
        }
        Cmd::Image(i) => {
            let body = ImageUpdate {
                path: i.path, clear: if i.clear { Some(true) } else { None },
                pan_x: i.pan_x, pan_y: i.pan_y, opacity: i.opacity, blur: i.blur, scale: i.scale,
            };
...
fn resolve_cfg(o: &ServerCfg, need_auth: bool) -> Result<Cfg> {
    let sess = helio_core::Session::load(&helio_core::session_path(&helio_core::home())).unwrap_or_default();
    let pick = |flag: &Option<String>, fallback: String| {
        flag.clone().filter(|s| !s.trim().is_empty()).unwrap_or(fallback)
    };
    let server = pick(&o.server, sess.registration.server).trim_end_matches('/').to_string();
...
/// Create-or-reuse a collection by name (with an optional description) → its id.
fn create_collection(http: &reqwest::blocking::Client, c: &Cfg, name: &str, description: &str) -> Result<String> {
    ...
    Ok(v["id"].as_str().context("server returned no collection id")?.to_string())
}
...
fn upload_track(http: &reqwest::blocking::Client, c: &Cfg, id: &str, path: &Path) -> Result<serde_json::Value> {
    let name = path.file_stem().and_then(|s| s.to_str()).context("bad filename")?.to_string();
...
fn set_cover(http: &reqwest::blocking::Client, c: &Cfg, id: &str, image: &Path) -> Result<()> {
    let ext = image.extension().and_then(|s| s.to_str()).unwrap_or("").to_lowercase();
...
fn run_collection(cmd: CollectionCmd) -> Result<()> {
    ...
    match cmd {
        ...
        CollectionCmd::Create { name, description, cfg } => {
            let c = resolve_cfg(&cfg, true)?;
            let id = create_collection(&http, &c, &name, description.as_deref().unwrap_or(""))?;
            println!("  {}  {}  {}", ui::green("✓"), ui::white(&name), ui::dim(&id));
        }
        CollectionCmd::Upload { folder, name, description, ext, cover, cfg } => {
            let c = resolve_cfg(&cfg, true)?;
            ...
            let mut files: Vec<PathBuf> = std::fs::read_dir(&folder)
                ...
                .filter(|p| p.is_file())
                .filter(|p| {
                    let e = p.extension().and_then(|s| s.to_str()).unwrap_or("").to_lowercase();
                    let is_audio = matches!(e.as_str(), "mp3" | "flac" | "wav");
                    let ok_filter = want.as_ref().map_or(true, |w| &e == w);
                    is_audio && ok_filter
                })
                .collect();
            files.sort();
            ...
            for (i, f) in files.iter().enumerate() {
                ...
                print!("   {} {:>2}/{}  {:<24} {:>9}  … ",
                    ui::dim("↑"), i + 1, total, ui::white(fname), ui::grey(&human_size(sz)));
                flush();
                match upload_track(&http, &c, &id, f) {
                    Ok(v) => {
                        done += 1;
                        let d = v["track"]["durationSec"].as_u64().map(fmt_hms).unwrap_or_default();
            ...
            println!("\n  {}  {}/{} tracks live on {}\n",
                ui::green("done"), done, total, ui::white(&c.server));
        }
        ...
        CollectionCmd::Rm { id, track, cfg } => {
            ...
            let (u, what) = match &track {
                Some(t) => (format!("{}/collections/{}/tracks/{}", c.server, id, t), "track removed"),
                None => (format!("{}/collections/{}", c.server, id), "collection removed"),
            };
            ok_json(http.delete(u).header("Authorization", format!("Bearer {}", c.token)).send()?)?;
            println!("  {}  {}", ui::green("✓"), ui::grey(what));
        }
        CollectionCmd::Edit { id, name, description, track, title, cfg } => {
            let c = resolve_cfg(&cfg, true)?;
            ...
            match (&track, &title) {
                (Some(t), Some(ti)) => {
                    body.insert("tracks".into(), serde_json::json!([{ "id": t, "name": ti }]));
                }
            ...
            if body.is_empty() {
                anyhow::bail!("nothing to edit — pass --name, --description, or --track with --title");
            }
            ok_json(
...
fn meter(v: f32) -> String {
    ...
    let code = if db >= -3.0 { ui::RED } else if db >= -12.0 { ui::AMBER } else { ui::GREEN };
    let bar = format!("{}{}", ui::paint(code, &"█".repeat(f)), ui::dim(&"░".repeat(w - f)));
    let label = if v > 0.0001 { format!("{db:>4.0} dB") } else { " -inf".to_string() };
...
fn render_status(s: &Status) -> String {
    ...
    o += &format!("   {}{}\n", state,
        if s.on_air { ui::dim(&format!("   ·  uptime {uptime}  ·  {} reconnects", s.reconnects)) } else { String::new() });
    ...
    // PRESENCE
    o += &format!("\n{}\n", rule("PRESENCE"));
    let pres = if s.away { ui::amber("●  AWAY") } else { ui::green("●  ON DECK") };
    o += &format!("   {}\n", pres);

    // INPUT
    o += &format!("\n{}\n", rule("INPUT"));
    let dev = if s.input_device.is_empty() { "(system default)".to_string() } else { s.input_device.clone() };
...
fn print_devices(d: &DevicesResponse) {
    print!("{}", banner());
    println!("\n{}", rule("INPUTS"));
    let sel = if d.selected.is_empty() { "(system default)".to_string() } else { d.selected.clone() };
    ...
    for dev in &d.devices {
        ...
        if dev.is_current { tags.push(ui::green("● capturing")); }
        if dev.is_default { tags.push(ui::grey("default")); }
        let tagstr = if tags.is_empty() { String::new() } else { format!("   {}", tags.join(ui::dim(" · ").as_str())) };
        let mark = if dev.is_current { ui::green("▸") } else { ui::dim("·") };
...
fn ack(what: &str, resp: reqwest::blocking::Response) -> Result<()> {
    ...
    if a.ok {
        ...
        eprintln!("  {}  {} {}", ui::red("✗"), ui::grey(what), a.error.unwrap_or_default());
        std::process::exit(1);
    }
    Ok(())
}
...
#[allow(dead_code)]
fn _tty() -> bool { std::io::stdout().is_terminal() }
